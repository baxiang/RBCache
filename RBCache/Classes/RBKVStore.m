...
- (instancetype)initWithDirectory:(NSString *)directory
{
    if (self = [super init]) {
        ...
        db = [[RBKVDataBase alloc] initWithDirectory:databaseDirectory];
        if (!db) {
...
- (RBKVItem *)getItemForKey:(NSString *)key
{
    if (0 == key.length) return nil;
    RBKVItem *item = [db dbGetItemForKey:key];
    if (item) {
        if (![item isValid]) {
...
- (BOOL)removeItemsWithSizeLimit:(NSInteger)size
{
    ...
    int rowNum = 20;
    BOOL success = YES;
    NSArray<RBKVItem *> *items = [db dbGetItemsOrderByExpireDateAscWithLimit:rowNum];
    while (total > size && items.count > 0 && success) {
        for (RBKVItem *item in items) {
            if (total > size) {
                if (item.filename) {
                    [self cacheDeleteFile:item.filename];
                }
